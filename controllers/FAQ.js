...
export const Create_FAQ = async (req, res) => {
  try {
    ...
    console.log("Posting FAQ with question:", question, "and answer:", answer);
    ...
    console.log(`Cache cleared for faqs`);
...
export const GetAllFaqs = async (req, res) => {
  try {
    ...
    console.log("Fetching FAQs for targetLanguage");

    // Check Redis cache first
    const cachedFaqs = await redisClient.get(`faqs:${targetLanguage}`);
    if (cachedFaqs) {
      console.log("Found cached FAQs for targetLanguage");
      return res.status(200).json({
        translatedFaqs: JSON.parse(cachedFaqs),
      });
    }

    console.log("No cached FAQs found,now we will fetch from db");
